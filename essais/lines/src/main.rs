...
fn main() -> io::Result<()> {
    ...
    let re_sem = Regex::new(r"(?m)^([A-Za-zÄÖÜäöü].*,)?").unwrap(); // with flag multiline
    let re_com = Regex::new(r",").unwrap(); // with flag multiline
  ...
  //  let re_note_com = Regex::new(r"\[(.+)\]\((http.?://.+)\) (.+[^#note:])? ?(#note:.+)#?").unwrap();
    ...
    let mut modified_content = re_com.replace_all(&modified_content, ".");
