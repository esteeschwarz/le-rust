   ...
   // let masterpassword = &login_data.masterpassword;
    ...
    match check_credentials(&conn, table_name, password) {
        ...
        Ok(false) => {
             ...
             let response = HttpResponse::Unauthorized().body("Invalid credentials. Create new table?");
             response
             // Execute `check_create_pwd` logic
             match check_create_pwd(&conn, table_name, password, masterpassword) {
                 Ok(true) => println!("Master password is valid. Proceed to create table."),
                 Ok(false) => println!("Invalid master password."),
                 Err(_) => println!("Database error during master password check."),
             }
            }
        // HttpResponse::Unauthorized().body("Invalid credentials. Create new table?"),
