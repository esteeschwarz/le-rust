...
struct Args {
    ...
    /// post ID
    #[clap(short = 'k', long, default_value = "common")]
...
struct WordPressConfig {
    ...
    blogurl: String,
    // postid: Option<String>
    postid: String //TODO: maybe that one out for post issue
}
...
fn extract_category(markdown: &str) -> Option<String> {
    for line in markdown.lines() {
        if line.starts_with("@cat:") {
            return Some(line.trim_start_matches("@cat:").trim().to_string());
        }
    }
    None
}
...
fn remove_cat(markdown: &str) -> String {
    let mut lines = markdown.lines();
    let mut result = String::new();

    // Skip the first line if it's a level 1 heading
    if let Some(first_line) = lines.next() {
        if !first_line.starts_with("@cat:") {
            result.push_str(first_line);
            result.push('\n');
        }
    }
...
fn set_category(url: &str, blog_id: i32, username: &str, password: &str, post_id: &str, post_category: &str ) -> Result<String, reqwest::Error> {
    // let blog_id = 1;
     let params = vec![
         //"1".to_string(),
         format!("<value><string><![CDATA[{}]]></string></value>",blog_id.to_string()),
         format!("<value><string><![CDATA[{}]]></string></value>",username.to_string()),
         format!("<value><string><![CDATA[{}]]></string></value>",password.to_string()),
         format!("<struct><member><name>description</name><value><string><![CDATA[{}]]></string></value></member><member><name>category</name><value><string><![CDATA[{}]]></string></value></member></struct>",content.to_string(),post_title.to_string()),
         format!("<value><boolean>{}</boolean></value>", if publish { 1 } else { 0 }),
 
 
     ];
     xml_rpc_call(url, "mt.setPostCategories", params)
 }
...
fn main() {
    ...
    let markdown_without_cat = remove_cat(&markdown_without_h1);

    // Convert the modified Markdown content to HTML
    let html_content = markdown_to_html(&markdown_without_cat);
    ...
    match args.method.as_str() {
        "editPost" => {
            ...
            let post_cat = extract_category(&markdown_content)
        ...
        "newPost" => {
            ...
            let xml: Response = from_str(response).unwrap();

            for item in xml.value {
                println!("postid: {}\n", item.string);
